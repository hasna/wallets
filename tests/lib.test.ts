...
import { closeDatabase, getDatabase, resetDatabase } from '../src/db/database';
import { createProvider, ensureProvider } from '../src/db/providers';
import {
	getConfigDir,
	getConfigPath,
	getProviderConfig,
...
describe('config', () => {
	...
	it('setProviderConfig saves provider config', () => {
		ensureProvider('agentcard', 'agentcard', {});
	...
	it('removeProviderConfig removes config', () => {
		ensureProvider('agentcard2', 'agentcard', {});
		setProviderConfig('agentcard2', { jwt: 'tok' });
		removeProviderConfig('agentcard2');
		const config = getProviderConfig('agentcard2');
		expect(config).toEqual({});
	});
...
describe('formatError', () => {
	...
	it('formats InsufficientFundsError', () => {
		const result = JSON.parse(
			formatError(new InsufficientFundsError('c', 1, 0))
		);
...
describe('doctor', () => {
	...
	it('checks agentcard provider health', () => {
		...
		createProvider(
			{ name: 'ac', type: 'agentcard', config: { jwt: 'tok' } },
			db
		);
		const result = runDoctor();
