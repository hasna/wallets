...
import { getDatabase, closeDatabase, resetDatabase, now, uuid, shortId, resolvePartialId } from "../src/db/database";
import { createProvider, getProvider, getProviderByName, getProviderByType, listProviders, updateProvider, deleteProvider, ensureProvider } from "../src/db/providers";
import { createCardRecord, getCard, getCardByExternalId, listCards, updateCard, updateCardBalance, deleteCard } from "../src/db/cards";
import { createTransaction, getTransaction, listTransactions } from "../src/db/transactions";
import { registerAgent, getAgent, getAgentByName, listAgents, deleteAgent } from "../src/db/agents";
...
describe("providers", () => {
  it("creates a provider", () => {
    const provider = createProvider({ name: "agentcard", type: "agentcard" }, db);
  ...
  it("creates provider with config", () => {
    const provider = createProvider({ name: "ac", type: "agentcard", config: { jwt: "tok123" } }, db);
  ...
  it("updates provider", () => {
    const created = createProvider({ name: "test", type: "test" }, db);
    const updated = updateProvider(created.id, { config: { key: "val" }, status: "inactive" }, db);
...
describe("cards", () => {
  ...
  it("gets card by id", () => {
    const card = createCardRecord({ provider_id: providerId, external_id: "e1", name: "C" }, db);
  ...
  it("gets card by external id", () => {
    createCardRecord({ provider_id: providerId, external_id: "ext-unique", name: "C" }, db);
  ...
  it("lists cards with filters", () => {
    createCardRecord({ provider_id: providerId, external_id: "a", name: "A", status: "active" }, db);
    createCardRecord({ provider_id: providerId, external_id: "b", name: "B", status: "closed" }, db);
    createCardRecord({ provider_id: providerId, external_id: "c", name: "C", status: "active", currency: "EUR" }, db);
  ...
  it("updates card", () => {
    const card = createCardRecord({ provider_id: providerId, external_id: "u1", name: "Up" }, db);
    const updated = updateCard(card.id, { name: "Updated", status: "active" }, db);
    expect(updated.name).toBe("Updated");
    expect(updated.status).toBe("active");
  });

  it("updates card metadata", () => {
    const card = createCardRecord({ provider_id: providerId, external_id: "m1", name: "M" }, db);
  ...
  it("updates card balance", () => {
    const card = createCardRecord({ provider_id: providerId, external_id: "b1", name: "B", balance: 50 }, db);
    const updated = updateCardBalance(card.id, 75.5, db);
    expect(updated.balance).toBe(75.5);
  });

  it("deletes card", () => {
    const card = createCardRecord({ provider_id: providerId, external_id: "d1", name: "D" }, db);
...
describe("transactions", () => {
  ...
  beforeEach(() => {
    ...
    providerId = provider.id;
    const card = createCardRecord({ provider_id: providerId, external_id: "tc1", name: "TC" }, db);
    cardId = card.id;
  });
  ...
  it("gets transaction by id", () => {
    const tx = createTransaction({ card_id: cardId, provider_id: providerId, type: "load", amount: 100 }, db);
  ...
  it("lists transactions with filters", () => {
    createTransaction({ card_id: cardId, provider_id: providerId, type: "purchase", amount: 10 }, db);
    createTransaction({ card_id: cardId, provider_id: providerId, type: "refund", amount: 5 }, db);
    createTransaction({ card_id: cardId, provider_id: providerId, type: "purchase", amount: 20, status: "failed" }, db);
...
describe("agents", () => {
  ...
  it("registers agent with description", () => {
    const agent = registerAgent({ name: "titus", description: "wallet manager" }, db);
