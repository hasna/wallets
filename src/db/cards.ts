...
import type { Card, CardRow, CardStatus, ListCardsFilter, UpdateCardInput } from "../types/index.js";
...
export function getCard(id: string, db?: Database): Card | null {
  const d = db || getDatabase();
  const row = d.query("SELECT * FROM cards WHERE id = ?").get(id) as CardRow | null;
...
export function listCards(filter: ListCardsFilter = {}, db?: Database): Card[] {
  ...
  const rows = d.query(`SELECT * FROM cards ${where} ORDER BY created_at DESC ${limit} ${offset}`).all(...params) as CardRow[];
  return rows.map(rowToCard);
}

export function updateCard(id: string, input: UpdateCardInput, db?: Database): Card {
  const d = db || getDatabase();
...
export function updateCardBalance(id: string, balance: number, db?: Database): Card {
  const d = db || getDatabase();
