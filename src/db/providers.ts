...
import { cacheClear, cached } from '../lib/cache.js';
import type {
	CreateProviderInput,
	Provider,
	ProviderRow,
...
export function getProviderByType(
	...
	const row = d
		.query(
			"SELECT * FROM providers WHERE type = ? AND status = 'active' LIMIT 1"
		)
		.get(type) as ProviderRow | null;
	return row ? rowToProvider(row) : null;
}
...
export function updateProvider(
	id: string,
	input: Partial<CreateProviderInput> & { status?: Provider['status'] },
	db?: Database
): Provider {
	const d = db || getDatabase();
